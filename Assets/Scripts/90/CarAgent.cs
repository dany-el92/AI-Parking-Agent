...
public class CarAgent : Agent {
    ...
    void FixedUpdate() {
        ...
        if(isPositioning && findParkingSpot) { 
            PositionCar(-1f); // Posiziona la macchina
        }

        // Se non si sta cercando un posto e ci sono dei target
    ...
    private void Reset() {
        ...
        float spawnZ = Random.Range((startPosition.z + 2.4f) - spawnRadiusZ, (startPosition.z + 2.4f) + spawnRadiusZ);
    ...
    private void FindParkingSpot() {
        ...
        if(RpMeasurements.RDistR[2] > 0.8f) {
        ...
        // Se i sensori laterali sinistri e destri rilevano due strisce allora 
        // la macchina si trova in uno spazio di parcheggio libero 
    ...
    private (float[] RDistL, float[] RDistR, float RDistF, float RDistB, bool hitStripeLF, bool hitStripeLB, bool hitStripeRF, bool hitStripeRB, bool hitPlantL, bool hitPlantR, bool hitRoadL, bool hitRoadR) RayPerceptionMeasurements() {
        ...
        // Utili per la ricerca del parcheggio
        bool hitStripeLeftFront = RayOutputs[4].HasHit && RayOutputs[4].HitGameObject != null && RayOutputs[4].HitGameObject.CompareTag("Stripe"); // Sensore anteriore sinistro 
        bool hitStripeLeftBack = RayOutputs[8].HasHit && RayOutputs[8].HitGameObject != null && RayOutputs[8].HitGameObject.CompareTag("Stripe"); // Sensore posteriore sinistro
        ...
        bool hitStripeRightFront = RayOutputs[3].HasHit && RayOutputs[3].HitGameObject != null && RayOutputs[3].HitGameObject.CompareTag("Stripe"); // Sensore anteriore destro
        bool hitStripeRightBack = RayOutputs[7].HasHit && RayOutputs[7].HitGameObject != null && RayOutputs[7].HitGameObject.CompareTag("Stripe"); // Sensore posteriore destro
    ...
    public float CalculateReward() {
        ...
        if(inTarget && targets != null && targets.Count > 0) {
            ...
            if(closestTarget != null) {
                ...
                if(angleToTarget < 20f && distanceToTarget < 1f && Mathf.Abs(carController.CurrentSpeed) < speed) {
                ...
                if(angleToTarget > 20f && inTarget && Mathf.Abs(carController.CurrentSpeed) < 0.5f) {
                    // L'agente è nel target ma non è allineato correttamente e non si sta muovendo molto
                    reward = 0f;
                }
